...
jQuery(document).ready(function($) {
    ...
    // bind clear button to clear values (also sets inputs to current timestamp)
    ...
    function iwUpdateTimestampText(field, validate) {
        ...
        var clearForm = (!validate && !$('#'+field).val());
;
        ...
        if ( attemptedDate.getFullYear() != aa || 
            ...
            if (validate == true) {
                ...
                $('.iw-cptsave').attr('disabled', 'disabled');
                return false;
            }
            clearForm = true;
        }
        $(div).removeClass('form-invalid');
        $('.iw-cptsave').removeAttr('disabled');
        if (clearForm) {
            $('#'+field+'_timestamp').html('');
            $('#'+field).val('');
        } else {
